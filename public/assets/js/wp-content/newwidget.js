(function ($) {
    ...
    function initMovingProjectItem($holder, $holder_id) {
      ...
      try {
        ...
        const firstRow = document.querySelector(
          $holder_id + " .mk-moving-projects-holder .mk-moving-projects-images:first-of-type"
        );
        const secondRow = document.querySelector(
          $holder_id + " .mk-moving-projects-holder .mk-moving-projects-images:nth-of-type(2)"
        );
        const projectImage = document.querySelector(
          $holder_id + " .mk-moving-projects-holder .mk-moving-projects-images:first-of-type .mk-moving-projects-other-img"
        );
        ...
        if (!firstRow || !secondRow || !projectImage) {
          console.error("Elements not found:", { firstRow, secondRow, projectImage });
          return;
        }
        ...
        const circleBg = projectInfo.querySelector($holder_id + " .mk-moving-project-info-bg");
        const iconImage = projectInfo.querySelector($holder_id + " .stat-main-title span");
        const imageItem = gsap.utils.toArray($holder_id + " .mk-moving-projects-img");
        const gsapAnim = gsap.timeline({ repeat: -1 });
  
        if (!projectInfo || !circleBg || !iconImage) {
          console.error("Hover elements not found:", { projectInfo, circleBg, iconImage });
          return;
        }
  
        gsapAnim.to(iconImage, { duration: 0.3, scale: 1.3, delay: 0.5, ease: "power3.in" });
        ...
        imageItem.forEach((imageElements) => {
          imageElements.addEventListener("mouseenter", function () {
            ...
            const dataStat = document.querySelector('[data-stat="' + number + '"]');
  
            gsapAnim.pause();
            gsap.to(dataStat, { yPercent: -50, opacity: 1, scale: 1, duration: 0.4, ease: "power4" });
          ...
          imageElements.addEventListener("mouseleave", function () {
            ...
            gsap.to(dataStat, { yPercent: -50, opacity: 0, scale: 0.5, duration: 0.4, ease: "power4" });
    ...
    mkAddons.widgetsList.mk_moving_projects = { mkMovingProjects: initMkMovingProjects };
